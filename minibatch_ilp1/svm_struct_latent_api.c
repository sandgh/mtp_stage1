...
void setConfigPath() {
	//Sandip - Configurable Lib Path
	LD_LIBRARY_PATH = (char*) malloc(300);
	JAVA_LIBRARY_PATH = (char*) malloc(300);

	FILE *fp_config = fopen("config.ini", "r");
	if (fp_config == NULL) {
		printf("Cannot open config.ini file %config.ini!\n");
		exit(1);
	}
...
void runTestCommand(char *modelfile ) {
	char *cmd = malloc(1000);
	strcpy(cmd, "export ");
	strcat(cmd, LD_LIBRARY_PATH);
	strcat(cmd," && java -Xmx4G -cp java/lib/*:java/bin/ evaluation.ClassifyStructEgAllOnline ");
	strcat(cmd, modelfile);
	strcat(cmd, " dataset/testSVM.pos_r.data dataset/reidel_mapping ");

	printf("Executing cmd : %s\n", cmd);fflush(stdout);
	system(cmd);
	free(cmd);

	char *cmd1 = malloc(1000);
	strcpy(cmd1, "export ");
	strcat(cmd1, LD_LIBRARY_PATH);
	strcat(cmd1," && java -Xmx4G -cp java/lib/*:java/bin/:/usr/lib/lp_solve/ evaluation.ReidelEval ");
	strcat(cmd1, modelfile);
	strcat(cmd1, ".result 0.5 >>");
	strcat(cmd1, modelfile);
	strcat(cmd1, ".result.stats");
	printf("Executing cmd : %s\n", cmd1);fflush(stdout);
	system(cmd1);
	free(cmd1);
...
//			" tmpfiles/max_violator_all dataset/reidel_trainSVM.data 0.9 ";
	...
	strcpy(cmd, "export ");
	strcat(cmd, LD_LIBRARY_PATH);
	strcat(cmd, "  && java -Xmx8G -cp java/bin:java/lib/* ");
	strcat(cmd, JAVA_LIBRARY_PATH);
	strcat(cmd, " javaHelpers.FindMaxViolatorHelperAll ");

	// ON MONASH ....
//        strcpy(cmd,
////        "export LD_LIBRARY_PATH=~/lsvm_code/libs/lp_solve/:~/lsvm_code/libs/mosek.5/5/tools/platform/linux64x86/bin/ && "
//        "export LD_LIBRARY_PATH=/usr/lib/lp_solve/:/home/sandip/mosek/5/tools/platform/linux64x86/bin/ && "
//           " java -Xmx8G -cp java/bin:java/lib/* "
//	   //" -Djava.library.path=../../libs/x86-64_sles10_4.1/:../../libs/lp_solve "
//        " -Djava.library.path=/home/sandip/DownloadDirector/cplex_installation/cplex/bin/x86-64_sles10_4.1/:/usr/lib/lp_solve "
//	   " javaHelpers.FindMaxViolatorHelperAll ");
...
void write_to_file_params_t_online_distr(double *w_prev_epoch, int totalEpochs, int numChunks, long num_of_features, long total_number_rels, char *filename){
	...
	fprintf(fp, "==\n");
	for(eid = 0; eid < 1; eid++){
		for(chunkid = 0; chunkid < 1; chunkid++){

			double *w = w_prev_epoch;
...
void write_to_file_params_t_online_distr_avg(double **w_prev_epoch, int totalEpochs, int numChunks, long num_of_features, long total_number_rels, char *filename){
	...
	int eid, chunkid;
	fprintf(fp, "==\n");
	for(eid = 1; eid <= totalEpochs; eid++){
		for(chunkid = 0; chunkid < 1; chunkid++){

			double *w = w_prev_epoch[eid];
...
void write_struct_model_online_distr(char *file, double *w_prev_epoch, STRUCT_LEARN_PARM *sparm, int totalEpochs, int numChunks) {
/*
  Writes the learned weight vector sm->w_iters to file after training.
*/
	write_to_file_params_t_online_distr(w_prev_epoch, totalEpochs, numChunks, sparm->max_feature_key, sparm->total_number_rels, file);
}

void write_struct_model_online_distr_avg(char *file, double **w_prev_epoch, STRUCT_LEARN_PARM *sparm, int totalEpochs, int numChunks) {
/*
  Writes the learned weight vector sm->w_iters to file after training.
*/
	write_to_file_params_t_online_distr_avg(w_prev_epoch, totalEpochs, numChunks, sparm->max_feature_key, sparm->total_number_rels, file);
}
