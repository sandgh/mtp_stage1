...
while (abs(config.v_trn_best-v_prev) > .00001 &&  abs(config.v_trn_best-v_prev_prev) > .00001)  || iteration_no < 10
...
%     figure;plot(plot_c,plot_v_trn,'r-');
    ...
    config.w_0_cost=1+config.BETA^2-config.v_trn_best;
    config.w_1_cost=config.v_trn_best*Theta_micro_trn;
