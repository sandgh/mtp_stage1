...
elseif isdataset(a) | isdouble(a)      % train or classify
	...
	if ~strcmp(v.mapping_file,mfilename) % training (base classifiers are already trained)
		...
		elseif isstacked(v)                % stacked combiner
			...
			error('Classifier combination expected')
		                  ...
		                  % Find weights								
		L = find(e < 1-max(getprior(a))); % take classifiers better than prior
    ...
    w = setname(w,mapname);	
      ...
      nclassf = length(u.data);
      s = sprintf('Evaluating %i base classifiers: ',nclassf);
      prwaitbar(nclassf,s);
			for j=1:nclassf                 % weight them
  ...
  ismapping(a);
  if numel(v) > 1     % normm cannot handle scalar inputs
    u = v*normm;      % the weights
  else
    u = v;
  end
