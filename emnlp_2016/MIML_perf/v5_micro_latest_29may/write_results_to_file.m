...
fid_parameters_precision_cls = fopen([config.result_file_name, '_precision_cls_', num2str(config.sntnce_k_prcnt), '.txt'], 'a');
fid_parameters_recall_cls = fopen([config.result_file_name, '_recall_cls_', num2str(config.sntnce_k_prcnt), '.txt'], 'a');
...
for i=1:config.no_of_relns
    ...
    fprintf(fid_parameters_fsc_cls, '%f,',config.f_score(i,1));
     fprintf(fid_parameters_precision_cls, '%f,',config.precision(i,1));
    fprintf(fid_parameters_recall_cls, '%f,',config.recall(i,1));       
end

fprintf(fid_parameters_fsc_cls, '\n');
fprintf(fid_parameters_precision_cls, '\n');
fprintf(fid_parameters_recall_cls, '\n');

fprintf(fid_parameters_thresh, '%f,',config.thresholds);
