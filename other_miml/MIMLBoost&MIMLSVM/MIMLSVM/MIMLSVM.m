function [HammingLoss,RankingLoss,OneError,Coverage,Average_Precision,Outputs,Pre_Labels,tr_time,te_time]=MIMLSVM(train_bags,train_target,test_bags,test_target,ratio,svm,cost,config)
...
save('distance_matrix.mat', 'distance_matrix', '-v7.3');
% load('distance_matrix.mat');

[clustering,matrix_fai,num_iter]=MIML_cluster(num_cluster,distance_matrix);
save('clustering.mat', 'clustering');
save('matrix_fai.mat', 'matrix_fai');
% load('clustering.mat');
% load('matrix_fai.mat');
...
%         t=2;
%         gamma=svm.para;
%         str=['-q -t ',num2str(t),' -g ',num2str(gamma),' -c ',num2str(cost)];
%     else
...
%     time_stamp=cputime;
%     
%     [predict_label, accuracy, dec_values] = svmpredict(test_target(i,:)', test_data, model, ' -q ');
%     if(model.Label(1)==1)
