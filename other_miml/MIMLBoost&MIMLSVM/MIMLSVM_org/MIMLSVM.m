...
%      save('distance_matrix_full.mat', 'distance_matrix');
     load('distance_matrix_full.mat');
%      save('distance_matrix_small.mat', 'distance_matrix');
%      load('distance_matrix_small.mat');
     
%      [clustering,matrix_fai,num_iter]=MIML_cluster(num_cluster,distance_matrix); 
     
%      save('clustering_full.mat', 'clustering');
     load('clustering_full.mat');
%      save('matrix_fai_full.mat', 'matrix_fai');
     load('matrix_fai_full.mat');
...
%      save('test_data_full.mat','test_data');
     load('test_data_full.mat');
%      save('test_data_small.mat','test_data');
%      load('test_data_small');
...
%      g = [.0001,.0005, .001,.005, .01,.05,.1, .5, 1, 5, 10, 50, 100];
%      g = 5:15;
     g = 11;
     g_len = length(g);
     results = zeros(g_len, 6);
...
%          [predict_label, accuracy, dec_values] = svmpredict(test_target(i,:)', test_data, model);
     ...
     results(i1, :) =  [ g(1,i1), Average_Precision, f_macro,precision,recall,f_micro ];
